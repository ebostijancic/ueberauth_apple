defmodule Ueberauth.Strategy.Apple do
  ...
  def handle_callback!(%Plug.Conn{params: %{"code" => code} = params} = conn) do
    ...
    case Ueberauth.Strategy.Apple.OAuth.get_access_token([code: code], opts) do
      {:ok, token} ->
        token_params = Ueberauth.json_library().decode!(token.access_token)

        apple_user =
          Map.put(user, "uid", UeberauthApple.uid_from_id_token(token_params["id_token"]))
  ...
  """
  def info(%Plug.Conn{params: %{"user" => userblob}} = _conn) do
    case Ueberauth.json_library().decode(userblob) do
      {:ok, user} ->
        %Info{
          email: user["email"],
